...
from pdf_audit import PDFAudit as PDF
...
class CMDWriter:
    def __init__(self, report_name, url, email,
        ...
        self.report_folder = os.path.join(REPORTS_FOLDER, report_name)  # Top level
        self.destination_folder = ''  # Where we are saving the current report effort (Test type, browser type)
        ...
        # Feeder CSV, currently being processed
        # self.csv_file_path = ''
        ...
        # Progress check
        # self.row_count = 0  # Row count total
        # self.row_count_i = 0  # Row count current
        ...
        # If not RESTART write the original request
        if not url == '':
    ...
    def master_controller(self):
        ...
        for root, dirs, files in os.walk(os.path.join(self.report_folder, 'CSV')):
            for file in files:
                ...
                # self.file_name = file
                csv_file_path = os.path.join(self.report_folder, 'CSV', file)
                if self.AXEChrome or self.AXEFirefox or self.AXEEdge:
                    if not (csv_file_path.find('__pdf__') > -1) == True:
                        # CMDWriter.axe_controller(self)
                        t = Thread(target=CMDWriter.axe_controller, args=(self, csv_file_path))
                        t.setDaemon(True)
                        t.start()
                # Start Lighthouse CONTROLLER
                if self.LighthouseMOBILE or self.LighthouseDESKTOP:
                    if csv_file_path.find('__pdf__') < 0:
                        t = Thread(target=self.lighthouse_controller, args=(csv_file_path,))
                        t.setDaemon(True)
                        t.start()
                # Start PDF CONTROLLER for __pdf__ files
                if self.PDFAudit:
                    if csv_file_path.find('__pdf__') > -1:
                        self_object = self
                        # PDFAudit().pdf_csv(self_object, self.csv_file_path)
                        t = Thread(target=PDF, args=(self_object, csv_file_path))
                        t.setDaemon(True)
                        t.start()
    ...
    def axe_controller(self, csv_file_path):
        first_line = True
        # csv_path = os.path.join(self.destination_folder, 'internal_html.csv')
        # csv_path = self.CSVFile
        row_count = sum(1 for row in csv.reader(open(csv_file_path, 'r', encoding='utf8'), delimiter=','))
        row_count_i = row_count - 1

        # If log progress log exist, remove
        # if os.path.exists(os.path.join(self.logs, '_axe_controller_log.txt')):
        #     os.remove(os.path.join(self.logs, '_axe_controller_log.txt'))

        # Open HTML CSV list
        with open(csv_file_path, 'r', encoding='utf8') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            # should be CSV
            report_file_path = os.path.join(self.report_folder, 'AXE', 'AXE_CHROME_DETAILS.csv')
            # Check for completed URLs
            for row in csv_reader:
                if first_line:
                    first_line = False
                    continue
                # Compare AXE_REPORT to CSV to skip
                elif os.path.exists(report_file_path):
                    with open(report_file_path, encoding='utf8') as completed_urls:
                        ...
                        for completed_url in completed_urls_reader:
                            ...
                            if row[0] in completed_url:
                                ...
                                if row_count_i >= 0:
                                    utils.log_line(os.path.join(self.logs, '_axe_controller_log.txt'), ' '.join(msg))
                                print(msg)
                                break
                            else:
                                jump = False
                ...
                try:
                    ...
                    if self.AXEChrome:
                        ...
                        msg = (' >>> Remaining URLs for [AXE CHROME]:', str(row_count_i), 'out of', str(row_count),
                               str(datetime.datetime.now())[:-7])
                        print(' '.join(msg))
                        utils.log_line(os.path.join(self.logs, os.path.split(csv_file_path)[1][:-4] +
                                                    '_axe_chrome_log.txt'), ' '.join(msg))
                    ...
                    if self.AXEFirefox:
                        ...
                        # Log "Remaining URLs" to progress log
                        msg = (' >>> Remaining URLs for [AXE FIREFOX]:', str(row_count_i), 'out of', str(row_count),
                               str(datetime.datetime.now())[:-7])
                        print(' '.join(msg))
                        utils.log_line(os.path.join(self.logs, os.path.split(csv_file_path)[1][:-4] +
                                                    '_axe_firefox_log.txt'), ' '.join(msg))
                    ...
                    # Log "Remaining URLs" to progress log
                    msg = (' >>> Remaining URLs for [AXE EDGE]:', str(row_count_i), 'out of', str(row_count),
                           str(datetime.datetime.now())[:-7])
                    print(' '.join(msg))
                    utils.log_line(os.path.join(self.logs, os.path.split(csv_file_path)[1][:-4] +
                                                '_axe_edge_log.txt'), ' '.join(msg))
    ...
    def axeChrome(self, axe_url):
        try:
            ...
            driver.get(axe_url)
            # Start thread
            t = Thread(target=CMDWriter.axe_runner, args=(self, driver, axe_url, destination_folder, 'CHROME'))
            t.setDaemon(True)
            t.start()
            thread_monitor = Thread(target=CMDWriter.thread_monitor, args=('AXE RUNNER CHROME', t,))
            thread_monitor.setDaemon(True)
            thread_monitor.start()
    ...
    def axeFirefox(self, axe_url):
        try:
            ...
            driver.get(axe_url)
            # Start thread
            t = Thread(target=CMDWriter.axe_runner, args=(self, driver, axe_url, destination_folder, 'FIREFOX'))
            t.setDaemon(True)
            t.start()
            thread_monitor = Thread(target=CMDWriter.thread_monitor, args=('AXE RUNNER FIREFOX', t,))
            thread_monitor.setDaemon(True)
            thread_monitor.start()
    ...
    def axeEdge(self, axe_url):
        try:
            ...
            driver.get(axe_url)
            # Start thread
            t = Thread(target=CMDWriter.axe_runner, args=(self, driver, axe_url, destination_folder, 'EDGE'))
            t.daemon = True
            t.start()
    ...
    def lighthouse_controller(self, csv_file_path):
        first_line = True
        row_count = sum(1 for row in csv.reader(open(csv_file_path, 'r', encoding='utf8'), delimiter=','))
        row_count_i = row_count - 1
        # Open HTML list
            ...
            for row in csv_reader:
                ...
                try:
                    ...
                    while threading.active_count() > self.thread_limit:
                        ...
                        time.sleep(5)
                    t = Thread(target=CMDWriter.lighthouse, args=(self, row[0]))
                    t.daemon = True
                    t.start()
                    ...
                    print(' '.join(msg))
                    utils.log_line(os.path.join(self.logs, os.path.split(csv_file_path)[1][:-4] +
                                                '_lighthouse_progress_log.txt'), ' '.join(msg))
    ...
    def lighthouse(self, lighthouse_url):
        ...
        try:
            ...
            lighthouse_path = os.path.join(destination_folder, 'LIGHTHOUSE_DETAILS.csv')
